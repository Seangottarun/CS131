...
def match_descriptors(desc1, desc2, threshold=0.5):
    ...
    ### YOUR CODE HERE
    # dists is a M by N matrix with distances between each pair of descriptors
    # iterate through each row of dists matrix to compare keypoint i with its
    # neighbours
    for i in range(M):
        dist = dists[i, :]
        if M <= N:
            dist[i] = np.inf # ith value is distance between keypoint and itself
        if np.min(dist) / np.partition(dist, 1)[1] <= threshold:
...
def ransac(keypoints1, keypoints2, matches, n_iters=200, threshold=20):
    ...
    for i in range(n_iters):
        ...
        inliers = np.sqrt(((p_matched2.dot(H_est) - p_matched1) ** 2).sum(axis=-1)) < threshold
...
def hog_descriptor(patch, pixels_per_cell=(8,8)):
    ...
    for m in range(rows):
        for n in range(cols):
            ...
            for i in range(G_cells.shape[2]):
                for j in range(G_cells.shape[3]):
                    idx = int(theta_cells[m, n, i, j] // degrees_per_bin)
                    if idx == 9:
                        idx = 8
                    cells[m, n, idx] += G_cells[m, n, i, j]
