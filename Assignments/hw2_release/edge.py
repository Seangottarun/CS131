...
def gaussian_kernel(size, sigma):
    ...
    for i in range(size):
        for j in range(size):
            ...
            kernel[i, j] = coeff * np.exp(exponent)
    ### END YOUR CODE
...
def partial_x(img):
    ...
    ### YOUR CODE HERE
    x_deriv_filter = 0.5 * np.array([-1,0,1])
    x_deriv_filter.shape = 1, 3
    out = conv(img, x_deriv_filter)
    ### END YOUR CODE
...
def partial_y(img):
    ...
    ### YOUR CODE HERE
    y_deriv_filter = 0.5 * np.array([-1, 0, 1])
    y_deriv_filter.shape = 3, 1
    out = conv(img, y_deriv_filter)
    ### END YOUR CODE
...
def double_thresholding(img, high, low):
    ...
    strong_edges = G >= high
    # Note: & is elmentwise AND (bitwise) operation for 2 boolean arrays
