...
def get_eigen_values_and_vectors(M, k):
    ...
    ### YOUR CODE HERE
    w, v = eigen_decomp(M)
    asc_evals_args = np.argsort(np.abs(w))
    evals_args = asc_evals_args[::-1]

    sorted_eigenvalues = w[evals_args]
    sorted_eigenvectors = v[:,evals_args]

    eigenvalues = sorted_eigenvalues
    eigenvectors = sorted_eigenvectors[:k]
    ### END YOUR CODE
