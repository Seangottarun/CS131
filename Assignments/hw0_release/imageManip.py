...
def mix_images(image1, image2, channel1, channel2):
    ...
    out = np.concatenate((left[:, :width//2, :], right[:, width//2:, :]), axis=1)
    ### END YOUR CODE
...
def mix_quadrants(image):
    ...
    top_left = rgb_exclusion(np.copy(image), 'R')
    top_right = dim_image(np.copy(image))
    bottom_left = np.copy(image)**0.5
    bottom_right = rgb_exclusion(image, 'R')

    out[:height//2, :, :] = np.concatenate((top_left[:height//2, :width//2, :], top_right[:height//2, width//2:, :]), axis=1)
    out[height//2:, :, :] = np.concatenate((bottom_left[height//2:, :width//2, :], bottom_right[height//2:, width//2:, :]), axis=1)
    ### END YOUR CODE
